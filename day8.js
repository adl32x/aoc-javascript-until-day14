const input = `cdeba gadec dafcgb bedg bd cdb cdebga fadecg adcfbeg bacfe | aedgbcf gcfead dbacfeg gecabd
...
const beforeDelimeter = rows.map(r => r[0])
const afterDelimeter = rows.map(r => r[1])

const allAfterDelimeter = afterDelimeter.join(' ')

let answer = allAfterDelimeter.split(' ').filter(w => w.length === 2).length
answer += allAfterDelimeter.split(' ').filter(w => w.length === 3).length
answer += allAfterDelimeter.split(' ').filter(w => w.length === 4).length
answer += allAfterDelimeter.split(' ').filter(w => w.length === 7).length

console.log('Part 1:', answer)


String.prototype.minus = function (a) {
  return this.replace(new RegExp(`[${a}]`, 'g'), '')
}

String.prototype.plus = function (a) {
  return Array.from(new Set((this + a))).sort().join('')
}


function filterAndReturnFirst(arr, f) {
  const filtered = arr.filter(f)
  return [filtered[0], arr.filter(x => !f(x))]
}

function calculate([vocabulary, digits]) {
  
  const vocabularyWords = vocabulary.split(' ').map(v => v.split('').sort().join(''))
  const digit = digits.split(' ').map(d => d.split('').sort().join(''))

  const one = vocabularyWords.filter(w => w.length === 2)[0]
  const four = vocabularyWords.filter(w => w.length === 4)[0]
  const seven = vocabularyWords.filter(w => w.length === 3)[0]
  const eight = vocabularyWords.filter(w => w.length === 7)[0]
  const upper = seven.minus(one)

  const zeroSixOrNine = vocabularyWords.filter(w => w.length === 6)
  const [six, zeroOrNine] = filterAndReturnFirst(zeroSixOrNine, w => w.plus(one).length === 7)

  const bottomAndBottomLeft = eight.minus(four).minus(upper)

  const [zero, nineA] = filterAndReturnFirst(zeroOrNine, w => w.plus(bottomAndBottomLeft).length === 6)
  const nine = nineA[0]

  const twoThreeOrFive = vocabularyWords.filter(w => ![zero, one, four, six, seven, eight, nine].includes(w))
  const [two, threeOrFive] = filterAndReturnFirst(twoThreeOrFive, w => w.plus(bottomAndBottomLeft).length === 5)

  const [three, fiveA] = filterAndReturnFirst(threeOrFive, w => w.plus(one).length === 5)
  const five = fiveA[0]
  ...
  return parseInt(digit.map(v => values[v]).join(''))
}

console.log('Part 2:', rows.map(row => { return calculate(row) }).reduce((a, b) => a+b))